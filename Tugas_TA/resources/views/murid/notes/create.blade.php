<x-layouts.murid>
    <x-slot name="title">Tulis Catatan</x-slot>
    ...
    <div class="max-w-3xl mx-auto">
        <div class="mb-12">
            ...
            <h1 class="text-4xl font-black text-slate-900 tracking-tighter italic uppercase leading-none">Tulis Catatan Baru</h1>
            <p class="text-slate-500 font-bold text-lg mt-2 italic">Ekspresikan pemikiran dan pengetahuanmu.</p>
        </div>

        <form action="{{ route('murid.notes.store') }}" method="POST" class="space-y-8">
            @csrf
            ...
            <div class="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm space-y-6">
                <div>
                    ...
                    <input type="text" name="title" id="title" required placeholder="Contoh: Logika Perulangan di Python"
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="10" required placeholder="Tuliskan isi catatanmu di sini..."
                              class="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-slate-900 font-medium placeholder:text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all italic"></textarea>

<x-layouts.guru>
    <x-slot name="title">Edit Tugas: {{ $tuga->title }}</x-slot>

    <div class="max-w-2xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <form action="{{ route('guru.tugas.update', $tuga) }}" method="POST" enctype="multipart/form-data" class="p-8">
            @csrf
            @method('PUT')
            <div class="space-y-6">
                <div>
                    ...
                    <select name="classroom_id" id="classroom_id" required
                        ...
                        @foreach($classrooms as $classroom)
                            <option value="{{ $classroom->id }}" {{ old('classroom_id', $tuga->classroom_id) == $classroom->id ? 'selected' : '' }}>{{ $classroom->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $tuga->title) }}" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition">
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="6"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition">{{ old('description', $tuga->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    @if($tuga->file_path)
                        <div class="flex items-center p-3 bg-indigo-50 border border-indigo-100 rounded-lg mb-2">
                            ...
                            <span class="text-xs text-indigo-700">{{ basename($tuga->file_path) }}</span>
                        </div>
                    @else
                        <p class="text-xs text-gray-400 italic mb-2">Belum ada file pendukung.</p>
                    @endif

                    <label for="file" class="block text-sm font-bold text-gray-700 mb-2">Ganti File Pendukung (Opsional)</label>
                ...
                <div class="flex items-center">
                    ...
                    <input type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published', $tuga->is_published) ? 'checked' : '' }}
                        class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 transition">
                    <label for="is_published" class="ml-2 block text-sm text-gray-700">Publikasikan tugas</label>
                </div>
            </div>
            ...
            <div class="mt-8 pt-6 border-t border-gray-100 flex justify-end space-x-4">
                ...
                <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 shadow-md shadow-indigo-200 transition">Update Tugas</button>
            </div>
        </form>
    </div>
</x-layouts.guru>

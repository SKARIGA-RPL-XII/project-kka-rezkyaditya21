<x-layouts.guru>
    <x-slot name="title">Buat Kelas Baru</x-slot>

    <div class="max-w-2xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <form action="{{ route('guru.kelas.store') }}" method="POST" class="p-8">
            @csrf
            <div class="space-y-6">
                <div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition"
                        placeholder="Berikan penjelasan singkat tentang kelas ini...">{{ old('description') }}</textarea>
                </div>
            ...
            <div class="mt-8 pt-6 border-t border-gray-100 flex justify-end space-x-4">
                ...
                <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 shadow-md shadow-indigo-200 transition">Simpan Kelas</button>
            </div>
        </form>
    </div>
</x-layouts.guru>

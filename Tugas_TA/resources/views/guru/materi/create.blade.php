<x-layouts.guru>
    <x-slot name="title">Tambah Materi Baru</x-slot>
    ...
    <div class="max-w-2xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <form action="{{ route('guru.materi.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition"
                        placeholder="Contoh: Dasar-dasar HTML & CSS">
                </div>
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="6"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition"
                        placeholder="Tuliskan isi materi atau keterangan di sini...">{{ old('content') }}</textarea>
                </div>

                <div>
                    <label for="file" class="block text-sm font-bold text-gray-700 mb-2">Upload File (Opsional)</label>
                    <div class="relative border-2 border-dashed border-gray-300 rounded-lg p-6 hover:border-indigo-400 transition cursor-pointer bg-gray-50 flex flex-col items-center">
                        ...
                        <p class="text-xs text-gray-400 mt-1">PDF, DOC, DOCX, PPT, PPTX, ZIP, JPG, PNG</p>
                    </div>
                </div>
                ...
                <div class="flex items-center">
                    ...
                    <label for="is_published" class="ml-2 block text-sm text-gray-700">Publikasikan materi ke kelas</label>
                </div>
            </div>
            ...
            <div class="mt-8 pt-6 border-t border-gray-100 flex justify-end space-x-4">
                ...
                <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 shadow-md shadow-indigo-200 transition">Simpan Materi</button>
            </div>
        </form>
    </div>
</x-layouts.guru>
